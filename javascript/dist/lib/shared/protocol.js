...
var _capabilityValidators = _interopRequireDefault(require("./validators/capabilityValidators.js"));
...
class Protocol {
  ...
  constructor(options) {
    ...
    this.setNotificationHandler(_constants.NOTIFICATION_METHOD_NAMES.cancelled, notification => {
      const controller = this._requestHandlerAbortControllers.get(notification.params.requestId);
      if (controller) {
        controller.abort(notification.params.reason);
      }
    });
    this.setNotificationHandler(_constants.NOTIFICATION_METHOD_NAMES.progress, notification => {
      this._onprogress(notification);
    });
    this.setRequestHandler(_constants.REQUEST_METHOD_NAMES.ping,
  ...
  */
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this._options) {
        if (this._options.enforceStrictCapabilities === true) {
          _capabilityValidators.default.assertCapabilityForMethod(request.method, this._clientCapabilities);
        }
      }
      if (options) {
        if (options.signal) {
  ...
  async notification(notification) {
    ...
    _capabilityValidators.default.assertNotificationCapability(notification.method, this._capabilities);
  ...
  setRequestHandler(method, handler, validator = request => request) {
    _capabilityValidators.default.assertRequestHandlerCapability(method, this._capabilities);
